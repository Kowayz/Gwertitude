<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>GWERTITUDE | Paiement Sécurisé</title>
    
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Bodoni+Moda:ital,wght@0,400;0,500;0,600;0,700;0,900;1,400&family=Italiana&family=Crimson+Text:wght@300;400;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">

    <style>
        :root {
            --noise: url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.04'/%3E%3C/svg%3E");
            --pattern: url("data:image/svg+xml,%3Csvg viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cdefs%3E%3Cpattern id='luxuryPattern' patternUnits='userSpaceOnUse' width='30' height='30'%3E%3Ccircle cx='15' cy='15' r='0.8' fill='rgba(212,175,55,0.06)' /%3E%3C/pattern%3E%3C/defs%3E%3Crect width='100%25' height='100%25' fill='url(%23luxuryPattern)' /%3E%3C/svg%3E");
            
            --bg-body: #080808;
            --bg-card: #121212;
            --text-main: #FFFFFF;
            --text-sub: #A0A0A0;
            --accent: #D4AF37;
            --border: rgba(255, 255, 255, 0.08);
            --glass: rgba(20, 20, 20, 0.9);
        }

        * { margin: 0; padding: 0; box-sizing: border-box; outline: none; transition: 0.3s; }
        body { 
            font-family: 'Crimson Text', serif; 
            background-color: var(--bg-body); 
            background-image: linear-gradient(45deg, rgba(212,175,55,0.008) 0%, transparent 50%, rgba(212,175,55,0.008) 100%), var(--pattern), var(--noise); 
            background-attachment: fixed; 
            color: var(--text-main); 
            overflow-x: hidden; 
            box-shadow: inset 0 0 200px rgba(212,175,55,0.02);
            min-height: 100vh;
        }

        /* NAV */
        nav { 
            position: fixed; top: 0; width: 100%; padding: 20px 5%; 
            display: flex; justify-content: space-between; align-items: center; 
            background: var(--glass); backdrop-filter: blur(20px); 
            z-index: 1000; border-bottom: 1px solid var(--border); 
        }
        .logo { 
            font-family: 'Italiana', serif; font-weight: 400; 
            font-size: 1.6rem; letter-spacing: 2px; 
            color: var(--text-main); 
            text-shadow: 0 0 20px rgba(212, 175, 55, 0.5); 
        }
        .back-btn { 
            font-size: 0.9rem; text-transform: uppercase; 
            color: var(--text-sub); display: flex; 
            align-items: center; gap: 10px; 
            cursor: pointer;
        }
        .back-btn:hover { color: var(--accent); gap: 15px; }

        /* MAIN CONTAINER */
        .payment-container {
            max-width: 1000px;
            margin: 120px auto 50px;
            padding: 0 20px;
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 40px;
        }

        /* LEFT SIDE - FORM */
        .payment-form {
            background: var(--bg-card);
            border: 1px solid var(--border);
            border-radius: 20px;
            padding: 40px;
        }

        h1 {
            font-family: 'Bodoni Moda', serif;
            font-size: 2.5rem;
            font-weight: 700;
            margin-bottom: 10px;
            color: var(--accent);
        }

        .secure-badge {
            display: flex;
            align-items: center;
            gap: 8px;
            color: var(--text-sub);
            font-size: 0.9rem;
            margin-bottom: 30px;
        }

        .form-section {
            margin-bottom: 30px;
        }

        .form-section h3 {
            font-family: 'Bodoni Moda', serif;
            font-size: 1.3rem;
            margin-bottom: 15px;
            color: var(--text-main);
        }

        .input-group {
            margin-bottom: 20px;
        }

        label {
            display: block;
            font-size: 0.95rem;
            color: var(--text-sub);
            margin-bottom: 8px;
            text-transform: uppercase;
            letter-spacing: 1px;
            font-size: 0.8rem;
        }

        input, select {
            width: 100%;
            padding: 15px;
            background: var(--bg-body);
            border: 1px solid var(--border);
            border-radius: 10px;
            color: var(--text-main);
            font-family: 'Crimson Text', serif;
            font-size: 1.1rem;
        }

        input:focus, select:focus {
            border-color: var(--accent);
            box-shadow: 0 0 20px rgba(212, 175, 55, 0.1);
        }

        .input-row {
            display: grid;
            grid-template-columns: 2fr 1fr;
            gap: 15px;
        }

        .payment-methods {
            display: flex;
            gap: 10px;
            margin-bottom: 20px;
        }

        .payment-method {
            flex: 1;
            padding: 15px;
            background: var(--bg-body);
            border: 2px solid var(--border);
            border-radius: 10px;
            text-align: center;
            cursor: pointer;
            font-size: 1.5rem;
        }

        .payment-method.active {
            border-color: var(--accent);
            background: rgba(212, 175, 55, 0.1);
        }

        .submit-btn {
            width: 100%;
            padding: 18px;
            background: linear-gradient(135deg, var(--accent), #C4A030);
            border: none;
            border-radius: 10px;
            color: #000;
            font-family: 'Bodoni Moda', serif;
            font-size: 1.2rem;
            font-weight: 700;
            cursor: pointer;
            text-transform: uppercase;
            letter-spacing: 2px;
            box-shadow: 0 10px 30px rgba(212, 175, 55, 0.3);
        }

        .submit-btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 15px 40px rgba(212, 175, 55, 0.4);
        }

        /* RIGHT SIDE - SUMMARY */
        .order-summary {
            background: var(--bg-card);
            border: 1px solid var(--border);
            border-radius: 20px;
            padding: 40px;
            height: fit-content;
            position: sticky;
            top: 120px;
        }

        .order-summary h2 {
            font-family: 'Bodoni Moda', serif;
            font-size: 1.8rem;
            margin-bottom: 25px;
        }

        .order-item {
            display: flex;
            gap: 15px;
            margin-bottom: 20px;
            padding-bottom: 20px;
            border-bottom: 1px solid var(--border);
        }

        .item-image {
            width: 80px;
            height: 80px;
            border-radius: 10px;
            object-fit: cover;
        }

        .item-details h4 {
            font-family: 'Bodoni Moda', serif;
            font-size: 1.1rem;
            margin-bottom: 5px;
        }

        .item-price {
            color: var(--accent);
            font-weight: 600;
        }

        .price-breakdown {
            margin-top: 20px;
        }

        .price-line {
            display: flex;
            justify-content: space-between;
            margin-bottom: 12px;
            font-size: 1.05rem;
        }

        .price-line.total {
            font-family: 'Bodoni Moda', serif;
            font-size: 1.5rem;
            font-weight: 700;
            color: var(--accent);
            margin-top: 20px;
            padding-top: 20px;
            border-top: 2px solid var(--border);
        }

        /* SUCCESS MODAL */
        .modal {
            display: none;
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
            height: 100%;
            background: rgba(0, 0, 0, 0.9);
            backdrop-filter: blur(10px);
            z-index: 2000;
            justify-content: center;
            align-items: center;
        }

        .modal.show {
            display: flex;
        }

        .modal-content {
            background: var(--bg-card);
            border: 1px solid var(--accent);
            border-radius: 20px;
            padding: 50px;
            text-align: center;
            max-width: 500px;
            box-shadow: 0 20px 60px rgba(212, 175, 55, 0.3);
        }

        .success-icon {
            font-size: 4rem;
            color: var(--accent);
            margin-bottom: 20px;
        }

        .modal-content h2 {
            font-family: 'Bodoni Moda', serif;
            font-size: 2rem;
            margin-bottom: 15px;
        }

        .modal-content p {
            color: var(--text-sub);
            margin-bottom: 30px;
        }

        .modal-btn {
            padding: 15px 40px;
            background: var(--accent);
            border: none;
            border-radius: 10px;
            color: #000;
            font-family: 'Bodoni Moda', serif;
            font-weight: 700;
            cursor: pointer;
            text-transform: uppercase;
            letter-spacing: 2px;
        }

        @media (max-width: 768px) {
            .payment-container {
                grid-template-columns: 1fr;
            }
            .order-summary {
                position: relative;
                top: 0;
            }
        }
    </style>
</head>
<body>
    <nav>
        <a href="index.php" class="logo">GWERTITUDE</a>
        <a href="index.php" class="back-btn">
            <i class="fas fa-arrow-left"></i> Retour
        </a>
    </nav>

    <div class="payment-container">
        <!-- LEFT: PAYMENT FORM -->
        <div class="payment-form">
            <h1>Paiement</h1>
            <div class="secure-badge">
                <i class="fas fa-lock"></i>
                <span>Paiement 100% sécurisé</span>
            </div>

            <form id="paymentForm">
                <!-- Payment Method -->
                <div class="form-section">
                    <h3>Méthode de paiement</h3>
                    <div class="payment-methods">
                        <div class="payment-method active" data-method="card">
                            <i class="fas fa-credit-card"></i>
                        </div>
                        <div class="payment-method" data-method="paypal">
                            <i class="fab fa-paypal"></i>
                        </div>
                        <div class="payment-method" data-method="crypto">
                            <i class="fab fa-bitcoin"></i>
                        </div>
                    </div>
                </div>

                <!-- Card Details -->
                <div class="form-section">
                    <h3>Informations bancaires</h3>
                    <div class="input-group">
                        <label>Numéro de carte</label>
                        <input type="text" placeholder="1234 5678 9012 3456" maxlength="19" required>
                    </div>
                    <div class="input-row">
                        <div class="input-group">
                            <label>Date d'expiration</label>
                            <input type="text" placeholder="MM/AA" maxlength="5" required>
                        </div>
                        <div class="input-group">
                            <label>CVV</label>
                            <input type="text" placeholder="123" maxlength="3" required>
                        </div>
                    </div>
                </div>

                <!-- Billing Info -->
                <div class="form-section">
                    <h3>Informations de facturation</h3>
                    <div class="input-group">
                        <label>Nom complet</label>
                        <input type="text" placeholder="Jean Dupont" required>
                    </div>
                    <div class="input-group">
                        <label>Email</label>
                        <input type="email" placeholder="jean.dupont@example.com" required>
                    </div>
                    <div class="input-group">
                        <label>Adresse</label>
                        <input type="text" placeholder="123 Avenue des Champs-Élysées" required>
                    </div>
                    <div class="input-row">
                        <div class="input-group">
                            <label>Ville</label>
                            <input type="text" placeholder="Paris" required>
                        </div>
                        <div class="input-group">
                            <label>Code postal</label>
                            <input type="text" placeholder="75008" required>
                        </div>
                    </div>
                </div>

                <button type="submit" class="submit-btn">
                    <i class="fas fa-lock"></i> Confirmer le paiement
                </button>
            </form>
        </div>

        <!-- RIGHT: ORDER SUMMARY -->
        <div class="order-summary">
            <h2>Récapitulatif</h2>
            
            <div id="order-items">
                <!-- Items du panier chargés dynamiquement -->
            </div>

            <div class="price-breakdown">
                <div class="price-line">
                    <span>Sous-total</span>
                    <span id="subtotal">0 €</span>
                </div>
                <div class="price-line">
                    <span>TVA (20%)</span>
                    <span id="tax">0 €</span>
                </div>
                <div class="price-line">
                    <span>Frais de service</span>
                    <span id="service-fee">0 €</span>
                </div>
                <div class="price-line total">
                    <span>Total</span>
                    <span id="grand-total">0 €</span>
                </div>
            </div>
        </div>
    </div>

    <!-- SUCCESS MODAL -->
    <div class="modal" id="successModal">
        <div class="modal-content">
            <div class="success-icon">
                <i class="fas fa-check-circle"></i>
            </div>
            <h2>Paiement Confirmé !</h2>
            <p>Votre commande a été traitée avec succès.<br>Un email de confirmation vous a été envoyé.</p>
            <button class="modal-btn" onclick="window.location.href='index.php'">
                Retour à l'accueil
            </button>
        </div>
    </div>

    <script>
        // Charger le panier depuis localStorage
        function loadCart() {
            const cart = JSON.parse(localStorage.getItem('gwertitudeCart') || '[]');
            const orderItemsContainer = document.getElementById('order-items');
            
            console.log('Panier chargé:', cart); // Debug
            
            if (cart.length === 0) {
                orderItemsContainer.innerHTML = '<p style="color: var(--text-sub); text-align: center; padding: 40px 0;">Votre panier est vide.<br><br><a href="index.php" style="color: var(--accent); text-decoration: underline;">Retour aux achats</a></p>';
                return;
            }
            
            let subtotal = 0;
            orderItemsContainer.innerHTML = ''; // Vider d'abord
            
            cart.forEach(item => {
                subtotal += item.price;
                const priceFormatted = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(item.price);
                
                orderItemsContainer.innerHTML += `
                    <div class="order-item">
                        <img src="${item.img}" alt="${item.title}" class="item-image" onerror="this.src='https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?w=200'">
                        <div class="item-details">
                            <h4>${item.title}</h4>
                            <p class="item-price">${priceFormatted}</p>
                        </div>
                    </div>
                `;
            });
            
            // Calculs
            const tax = subtotal * 0.20;
            const serviceFee = Math.max(50000, subtotal * 0.004); // Min 50k ou 0.4%
            const total = subtotal + tax + serviceFee;
            
            // Affichage
            document.getElementById('subtotal').innerText = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(subtotal);
            document.getElementById('tax').innerText = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(tax);
            document.getElementById('service-fee').innerText = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(serviceFee);
            document.getElementById('grand-total').innerText = new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(total);
        }
        
        // Charger le panier au chargement de la page
        window.addEventListener('DOMContentLoaded', loadCart);
        
        // Payment method selection
        document.querySelectorAll('.payment-method').forEach(method => {
            method.addEventListener('click', function() {
                document.querySelectorAll('.payment-method').forEach(m => m.classList.remove('active'));
                this.classList.add('active');
            });
        });

        // Form submission (FAKE)
        document.getElementById('paymentForm').addEventListener('submit', function(e) {
            e.preventDefault();
            
            // Simulate processing
            const btn = document.querySelector('.submit-btn');
            const originalText = btn.innerHTML;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Traitement...';
            btn.disabled = true;

            setTimeout(() => {
                // Vider le panier après paiement
                localStorage.removeItem('gwertitudeCart');
                document.getElementById('successModal').classList.add('show');
                btn.innerHTML = originalText;
                btn.disabled = false;
            }, 2000);
        });

        // Card number formatting
        document.querySelector('input[placeholder*="1234"]').addEventListener('input', function(e) {
            let value = e.target.value.replace(/\s/g, '');
            let formattedValue = value.match(/.{1,4}/g)?.join(' ') || value;
            e.target.value = formattedValue;
        });

        // Expiry date formatting
        document.querySelector('input[placeholder="MM/AA"]').addEventListener('input', function(e) {
            let value = e.target.value.replace(/\D/g, '');
            if (value.length >= 2) {
                value = value.slice(0, 2) + '/' + value.slice(2, 4);
            }
            e.target.value = value;
        });
    </script>
</body>
</html>
